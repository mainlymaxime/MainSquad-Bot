...
async function handleCountingGame(
  ...
  try {
    logger.info(
      `🔢 Counting handler | ${message.author.tag}: ${message.content}`
    );
    ...
    // =====================================================
    // FOUT NUMMER / DEZELFDE PERSOON TWEE KEER
    // =====================================================

    if (invalidAttempt) {

      // BELANGRIJK:
      // Het foute bericht NIET verwijderen.
      // Het blijft gewoon zichtbaar in het counting-kanaal.
    ...
    await message
      ...
      .catch((error) => {
        logger.warn(
          'Could not add counting checkmark:',
          error
        );
      });
