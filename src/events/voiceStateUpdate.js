...
export default {
    ...
    async execute(oldState, newState, client) {
        ...
        // =========================================================
        // VOICE LOGGING
        // Runs independently from Join-to-Create.
        // =========================================================
        try {
            await logVoiceActivity(client, oldState, newState);
        } catch (error) {
            logger.error(
                `Error logging voice activity for guild ${guildId}:`,
                error
            );
        }
        ...
        try {
            ...
            logger.error(
                `Error in Join-to-Create voiceStateUpdate for guild ${guildId}:`,
                error
            );
        }
        ...
        async function handleVoiceJoin(client, state, config) {
            ...
            if (existingTempChannel) {
                const tempChannel =
                    state.guild.channels.cache.get(existingTempChannel);
        ...
        async function handleVoiceLeave(client, state, config) {
            ...
            const tempChannelInfo = await getTemporaryChannelInfo(
                client,
                state.guild.id,
                channel.id
            );
            ...
            if (oldState.channel) {
                ...
                if (tempChannelInfo) {
                    if (oldState.channel.members.size === 0) {
                        ...
                        tempChannelInfo.ownerId === oldState.member.id
                    ) {
                        const nextMember =
        ...
        async function createTemporaryChannel(
            ...
            try {
                ...
                const bitrate = clampVoiceBitrate(
                ...
                const channelName =
                    sanitizeVoiceChannelName(finalName);
                ...
                const tempChannel = await guild.channels.create({
                    ...
                    userLimit:
                        userLimit === 0 ? undefined : userLimit,
                    bitrate: bitrate,
    ...
    if (
        ...
        await logEvent({
            ...
            data: {
                title: '🔊 Voice Joined',
                lines: [
                    `**Member:** ${member.toString()} (${member.user.tag})`,
